...
class recruitment_admin_manage_candidate_profils__list_display extends NxPage
{
	function recruitment_admin_manage_candidate_profils__list_display($props,&$parentDesc,$aspect)
	{
	...
	{
					$searchData=
						$this->getRecord('rct_candidate_profil.search','page.search','request',false,null,null);
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					$this->getRecords('rct_candidate_profil.list','page.records','db',false,$searchData,null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/admin/manage_candidate_profils__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/admin/manage_candidate_profils__list_ajax',array(
								'fmethod' => 'toHTML',
