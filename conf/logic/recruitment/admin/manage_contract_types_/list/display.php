...
class recruitment_admin_manage_contract_types__list_display extends NxPage
{
	function recruitment_admin_manage_contract_types__list_display($props,&$parentDesc,$aspect)
	{
	...
	{
					$searchData=
						$this->getRecord('rct_contract_type.search','page.search','request',false,null,null);
					$searchDataVar='searchData';
					$this->setProperty("filterKey",(($_nx_rec0=${$searchDataVar})!=null?$_nx_rec0->getUrlKey(true):''));
					$this->getRecords('rct_contract_type.list','page.records','db',false,$searchData,null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/admin/manage_contract_types__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/admin/manage_contract_types__list_ajax',array(
								'fmethod' => 'toHTML',
