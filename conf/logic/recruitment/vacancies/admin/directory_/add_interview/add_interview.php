...
class recruitment_vacancies_admin_directory__add_interview_add_interview extends NxPage
{
	function recruitment_vacancies_admin_directory__add_interview_add_interview($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					{
						$this->setProperty('previous_operation.url',$_SERVER["PHP_SELF"].'?nxo=calendar_vacancy&amp;nxs=&amp;oid='.$this->getProperty("vacancy_oid","").'&amp;void='.(isset($_REQUEST["oid"])?$_REQUEST["oid"]:"").'&amp;vacancy_oid='.(isset($_REQUEST["oid"])?$_REQUEST["oid"]:"").'&amp;mode=ajax&amp;from_vac=1');
						$this->setProperty('submit.url',$_SERVER["PHP_SELF"].'?nxo=add_interview_process&amp;nxs=&amp;from_vac=1');
					}
					if((isset($_REQUEST["from_vac"])?$_REQUEST["from_vac"]:"") != 1 )
					{
						$this->setProperty('previous_operation.url',$_SERVER["PHP_SELF"].'?nxo=calendar_vacancy&amp;nxs=&amp;oid='.$this->getProperty("vacancy_oid","").'&amp;void='.(isset($_REQUEST["oid"])?$_REQUEST["oid"]:""));
