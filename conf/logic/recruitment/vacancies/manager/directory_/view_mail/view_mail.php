...
class recruitment_vacancies_manager_directory__view_mail_view_mail extends NxPage
{
	function recruitment_vacancies_manager_directory__view_mail_view_mail($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$this->setProperty("previous_operation_label",$this->getString("back_to_candidates"));
					$this->setProperty("vacancy_oid",(isset($_REQUEST["vacancy_oid"])?$_REQUEST["vacancy_oid"]:""));
					$this->setProperty("email_oid",(isset($_REQUEST["ref_oid"])?$_REQUEST["ref_oid"]:""));
					$this->getRecord('rct_email.view@recruitment','page.record','db',false,null,null);
					$this->getRecords('rct_email_attached_file.by_email@recruitment','attached_files.records','db',false,null,null);
					$this->setProperty('download.url',$_SERVER["PHP_SELF"].'?nxo=email_download&amp;nxs=');
					$this->setProperty('previous_operation.url',$_SERVER["PHP_SELF"].'?nxo=candidates&amp;nxs=&amp;oid='.$this->getProperty("vacancy_oid",""));
					$this->runView('/recruitment/vacancies/manager/directory__email_view',array(
						'fmethod' => 'toHTML',
