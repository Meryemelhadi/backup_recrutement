...
class recruitment_vacancies_manager_directory__list_prest_display extends NxPage
{
	function recruitment_vacancies_manager_directory__list_prest_display($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$_searchKey_=(($_nx_rec43=${$searchDataVar})!=null?$_nx_rec43->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/vacancies/manager/directory__list_prest',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/vacancies/manager/directory__list_ajax_prest',array(
								'fmethod' => 'toHTML',
