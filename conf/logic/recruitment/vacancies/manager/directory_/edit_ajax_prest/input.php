...
class recruitment_vacancies_manager_directory__edit_ajax_prest_input extends NxPage
{
	function recruitment_vacancies_manager_directory__edit_ajax_prest_input($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$this->getRecord('rct_prestataire_vacancy.record','page.record','db',false,null,null);
					$this->setProperty("page_message",$this->getString("page_message_edit"));
					$this->setProperty('back.url',$_SERVER["PHP_SELF"].'?nxo=list_prest&amp;nxs='.(isset($_REQUEST["filterKey"])?$_REQUEST["filterKey"]:"").'&amp;mode=ajax&amp;oid='.(isset($_REQUEST["vacancy_oid"])?$_REQUEST["vacancy_oid"]:""));
					$this->setProperty('submit.url',$_SERVER["PHP_SELF"].'?nxo='.$this->getProperty("app.operation","").'&amp;nxs=process&amp;vacancy_oid='.(isset($_REQUEST["vacancy_oid"])?$_REQUEST["vacancy_oid"]:""));
					$this->runView('/recruitment/vacancies/manager/directory__edit_prest',array(
						'fmethod' => 'toHTML',
