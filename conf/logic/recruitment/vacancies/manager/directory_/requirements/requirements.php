...
class recruitment_vacancies_manager_directory__requirements_requirements extends NxPage
{
	function recruitment_vacancies_manager_directory__requirements_requirements($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					$this->setProperty("current_operation",'requirements');
					...
					$this->getRecord('rct_vacancy.edit_fib','page.record','db',false,null,null);
					...
					$this->setProperty('submit_continue.url',$_SERVER["PHP_SELF"].'?nxo='.$this->getProperty("app.operation","").'&amp;nxs=process&amp;redirect=requirements');
					$this->setProperty('requirements.url',$_SERVER["PHP_SELF"].'?nxo=requirements&amp;nxs=&amp;oid='.(isset($_REQUEST["oid"])?$_REQUEST["oid"]:""));
					$this->runView('/recruitment/vacancies/manager/directory__requirements',array(
						'fmethod' => 'toHTML',
