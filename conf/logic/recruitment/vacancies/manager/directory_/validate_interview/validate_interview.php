...
class recruitment_vacancies_manager_directory__validate_interview_validate_interview extends NxPage
{
	function recruitment_vacancies_manager_directory__validate_interview_validate_interview($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$this->getRecord('rct_interview.validate','page.record','db',false,null,null);
					$this->setProperty('back_to_calendar.url',$_SERVER["PHP_SELF"].'?nxo=calendar&amp;nxs=&amp;oid='.$this->getProperty("vacancy_oid",""));
					$this->setProperty('accept.url',$_SERVER["PHP_SELF"].'?nxo='.$this->getProperty("app.operation","").'&amp;nxs=validate_interview_process&amp;validation_oid=1&amp;interview_oid='.$this->getProperty("interview_oid","").'&amp;vacancy_oid='.$this->getProperty("vacancy_oid","").'&amp;candidate_oid='.$this->getProperty("candidate_oid",""));
					$this->setProperty('refuse.url',$_SERVER["PHP_SELF"].'?nxo='.$this->getProperty("app.operation","").'&amp;nxs=validate_interview_process&amp;validation_oid=2&amp;interview_oid='.$this->getProperty("interview_oid","").'&amp;vacancy_oid='.$this->getProperty("vacancy_oid","").'&amp;candidate_oid='.$this->getProperty("candidate_oid",""));
					$this->runView('/recruitment/vacancies/manager/directory__interview_validate',array(
						'fmethod' => 'toHTML',
