...
class recruitment_vacancies_manager_directory__view_provider_view_provider extends NxPage
{
	function recruitment_vacancies_manager_directory__view_provider_view_provider($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$this->setProperty("previous_operation_label",$this->getString("back_to_providers"));
					$this->setProperty("vacancy_oid",(isset($_REQUEST["vacancy_oid"])?$_REQUEST["vacancy_oid"]:""));
					$this->setProperty("vacancy_provider_oid",(isset($_REQUEST["vacancy_provider"])?$_REQUEST["vacancy_provider"]:""));
					$this->getRecord('rct_vacancy_provider.view@recruitment','page.record','db',false,null,null);
					$this->getRecords('rct_vacancy_provider_attached_file.by_vacancy_provider@recruitment','attached_files.records','db',false,null,null);
					$this->setProperty('download.url',$_SERVER["PHP_SELF"].'?nxo=provider_download&amp;nxs=');
					$this->setProperty('previous_operation.url',$_SERVER["PHP_SELF"].'?nxo=recruitment_providers&amp;nxs=&amp;oid='.$this->getProperty("vacancy_oid",""));
					$this->runView('/recruitment/vacancies/manager/directory__provider_view',array(
						'fmethod' => 'toHTML',
