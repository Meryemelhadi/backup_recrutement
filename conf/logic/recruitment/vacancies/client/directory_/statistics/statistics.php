...
class recruitment_vacancies_client_directory__statistics_statistics extends NxPage
{
	function recruitment_vacancies_client_directory__statistics_statistics($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$_searchKey_=(($_nx_rec52=${$searchDataVar})!=null?$_nx_rec52->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					$records=
						$this->getRecords('','page.records','db',false,$searchData,null);
					(($__o=$this->getHandler('','')) ? $__o->load_stats($records) : null);
					$this->getRecords('','page.vacancies_records','db',false,null,null);
					$this->getRecords('','candidate_categories.records','db',false,null,null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/vacancies/client/directory__stat',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/vacancies/client/directory__stat_ajax',array(
								'fmethod' => 'toHTML',
