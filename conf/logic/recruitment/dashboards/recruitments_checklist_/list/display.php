...
class recruitment_dashboards_recruitments_checklist__list_display extends NxPage
{
	function recruitment_dashboards_recruitments_checklist__list_display($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$_searchKey_=(($_nx_rec73=${$searchDataVar})!=null?$_nx_rec73->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					$this->getRecords('rct_vacancy.dashboards.recruitments_checklist.list','page.records','handler',false,$searchData,null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/dashboards/recruitments_checklist__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/dashboards/recruitments_checklist__list_ajax',array(
								'fmethod' => 'toHTML',
