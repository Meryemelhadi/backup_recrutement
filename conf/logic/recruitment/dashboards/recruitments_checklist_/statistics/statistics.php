...
class recruitment_dashboards_recruitments_checklist__statistics_statistics extends NxPage
{
	function recruitment_dashboards_recruitments_checklist__statistics_statistics($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$_searchKey_=(($_nx_rec72=${$searchDataVar})!=null?$_nx_rec72->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/dashboards/recruitments_checklist__stat',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/dashboards/recruitments_checklist__stat_ajax',array(
								'fmethod' => 'toHTML',
