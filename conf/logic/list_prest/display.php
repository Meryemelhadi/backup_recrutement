...
class list_prest_display extends NxPage
{
	function list_prest_display($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
							...
							$_searchKey_=(($_nx_rec93=${$searchDataVar})!=null?$_nx_rec93->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
							...
							if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
							{
								...
								if($viewMode == 'page')
								{
									$this->runView('_list_prest',array(
										'fmethod' => 'toHTML',

										),null);
								}
								else {
									$this->runView('_list_ajax_prest',array(
										'fmethod' => 'toHTML',
