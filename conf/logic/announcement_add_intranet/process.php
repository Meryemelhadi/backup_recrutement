...
class announcement_add_intranet_process extends NxPage
{
	function announcement_add_intranet_process($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
							...
							$this->putRecord(
								$_nx_record_data52=array(
										'ann_title'=>$article_title,
										'ann_content'=>$article_content,
										'type'=>$article_type,
										'ref_oid'=>$this->getProperty("lastInsertId",""),),'','insert','db','Object',null);
							$this->setProperty("lastInsertId",
								$this->getLastInsertID('db'));
							$this->putRecord(
