...
?><div id="help" class="help"><div class="message">{var:msgHelp}</div></div><?php 
		...
		{if:#property:from_home#}
			<button type="button" class="btn btn-default button_back" onclick="document.location.href='{=back_home.url}'"><span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>&nbsp;&nbsp;
			{string:"back_to_home"}
		</button>
		{else}
		<button type="button" class="btn btn-default button_back" onclick="document.location.href='{=back_list.url}'"><span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>&nbsp;&nbsp;
			{string:"back"}
		</button>
		{/if}
		<span class="pm-button-sep"></span><button type="button" class="btn btn-default" onclick="PMFormSubmit.submit(this, dlg_submitContinueConfig, null, dlg_failureConfig); this.blur(); return false;"><span class="glyphicon glyphicon-floppy-save" aria-hidden="true"></span>&nbsp;&nbsp;
			{string:"submit_continue"}
		</button>
					...
					<fieldset id="group_2" class="field-group-bs">
						...
						<div id="field_{field.name}" class="form-group form-group-bs field-type-{field.type} {if:field.is_last} last_field{/if}">
							...
							<span class="required-field">*</span>{/if} :
									...
									<div class="form-control-bs" id="{field.name}-control">{field:announce_date}</div></div>{get_field:contract_type_oid}{/get_field}
						<div id="field_{field.name}" class="form-group form-group-bs field-type-{field.type} {if:field.is_last} last_field{/if}">
										<div class="control-label-wrapper-bs">
											<label for="{field.name}" class="control-label-bs control-label">{field.label:contract_type_oid}{if:field.isRequired}
							<span class="required-field">*</span>{/if} :
											</label></div>
									<div class="form-control-bs" id="{field.name}-control">{field:contract_type_oid}</div></div>{get_field:vacancy_project}{/get_field}
						<div id="field_{field.name}" class="form-group form-group-bs field-type-{field.type} {if:field.is_last} last_field{/if}">
										<div class="control-label-wrapper-bs">
											<label for="{field.name}" class="control-label-bs control-label">{field.label:vacancy_project}{if:field.isRequired}
							<span class="required-field">*</span>{/if} :
											</label></div>
									<div class="form-control-bs" id="{field.name}-control">{field:vacancy_project}</div></div>{get_field:app_connected}{/get_field}
						<div id="field_{field.name}" class="form-group form-group-bs field-type-{field.type} {if:field.is_last} last_field{/if}">
										<div class="control-label-wrapper-bs">
											<label for="{field.name}" class="control-label-bs control-label">{field.label:app_connected}{if:field.isRequired}
							<span class="required-field">*</span>{/if} :
											</label></div>
									<div class="form-control-bs" id="{field.name}-control">{field:app_connected}</div></div></div>
					</fieldset>
					<div class="form-actions-wrapper">
						<div class="pm-button-wrapper clear-both pm-bxm-top-button-wrapper">
							<div class="btn-group btn-group-width">
								<button type="button" class="btn btn-default right" onclick="PMFormSubmit.submit(this, dlg_submitContinueConfig, null, dlg_failureConfig); this.blur(); return false;">
