...
//]]>
	...
	function group($group) 
	{
		...
		<fieldset id="group_29" class="field-group-bs"><legend class="">{var:title}</legend><div class="form-group-content">
			{=_fields_}
		</div></fieldset>
		{/if}
		<?php 
...
//]]>
</script><div class="form-actions-wrapper"><div class="pm-button-wrapper clear-both pm-bxm-top-button-wrapper"><button type="button" class="btn btn-default right" onclick="PMFormSubmit.submit(this, window.dlg_vma_w_cfg, window.dlg_vma_s_cfg, window.dlg_vma_f_cfg); this.blur(); return false;"><span class="glyphicon glyphicon-floppy-save" aria-hidden="true"></span>&nbsp;&nbsp; {string:"send_cv"} 
					...
					</button></div></div>{get_field:proposed_to}{/get_field}<div id="field_{field.name}" class="form-group form-group-bs field-type-{field.type} {if:field.is_last} last_field{/if}"><div class="control-label-wrapper-bs"><label for="{field.name}" class="control-label-bs control-label">{field.label:proposed_to}{if:field.isRequired}<span class="required-field">*</span>{/if} :</label></div><div class="form-control-bs" id="{field.name}-control">{field:proposed_to}</div></div>{get_field:comment}{/get_field}<div id="field_{field.name}" class="form-group form-group-bs field-type-{field.type} {if:field.is_last} last_field{/if}"><div class="control-label-wrapper-bs"><label for="{field.name}" class="control-label-bs control-label">{field.label:comment}{if:field.isRequired}<span class="required-field">*</span>{/if} :</label></div><div class="form-control-bs" id="{field.name}-control">{field:comment}</div></div>
			{/form}
...
//<![CDATA[
	...
	window.dlg_vma_s_cfg = { title : "{string:'dlg_success_title'}", message : "{string:'dlg_success_content'}", viewMode : "ok", url : "{=edit.url}&mode=ajax", target : "content", labels : { ok : "{string:'dlg_success'}" } };
