...
class recruitment_admin_manage_email_samples__list_display extends NxPage
{
	function recruitment_admin_manage_email_samples__list_display($props,&$parentDesc,$aspect)
	{
	...
	{
					$searchData=
						$this->getRecord('rct_email_sample.search','page.search','request',false,null,null);
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					$this->getRecords('rct_email_sample.list','page.records','db',false,$searchData,null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/admin/manage_email_samples__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/admin/manage_email_samples__list_ajax',array(
								'fmethod' => 'toHTML',
