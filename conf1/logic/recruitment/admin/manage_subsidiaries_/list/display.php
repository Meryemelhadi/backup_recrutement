...
class recruitment_admin_manage_subsidiaries__list_display extends NxPage
{
	function recruitment_admin_manage_subsidiaries__list_display($props,&$parentDesc,$aspect)
	{
	...
	{
					$searchData=
						$this->getRecord('rct_subsidiary.search','page.search','request',false,null,null);
					$searchDataVar='searchData';
					$this->setProperty("filterKey",(($_nx_rec0=${$searchDataVar})!=null?$_nx_rec0->getUrlKey(true):''));
					$this->getRecords('rct_subsidiary.list','page.records','db',false,$searchData,null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/admin/manage_subsidiaries__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/admin/manage_subsidiaries__list_ajax',array(
								'fmethod' => 'toHTML',
