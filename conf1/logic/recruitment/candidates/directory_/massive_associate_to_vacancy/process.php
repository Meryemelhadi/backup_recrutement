...
class recruitment_candidates_directory__massive_associate_to_vacancy_process extends NxPage
{
	function recruitment_candidates_directory__massive_associate_to_vacancy_process($props,&$parentDesc,$aspect)
	{
	...
	{
					$candidates_oids=
						(($__o=$this->getHandler('lib.recruitment.candidates.services.directory','')) ? $__o->saveAssociations((isset($_REQUEST["candidates_oid"])?$_REQUEST["candidates_oid"]:""),(isset($_REQUEST["vacancy_oid"])?$_REQUEST["vacancy_oid"]:"")) : null);
					...
					header('Location:'.($u=$this->getProperty('site.url.noprefix','').$_SERVER["PHP_SELF"].'?nxo=list&nxs='.preg_replace(array('%&amp;%','%[+]%'),array('&','%2B'),urldecode(urlencode((isset($_REQUEST["filterKey"])?$_REQUEST["filterKey"]:"")))).'&mode=ajax&is_search=true'));
