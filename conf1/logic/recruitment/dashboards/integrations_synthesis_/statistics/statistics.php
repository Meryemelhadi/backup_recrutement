...
class recruitment_dashboards_integrations_synthesis__statistics_statistics extends NxPage
{
	function recruitment_dashboards_integrations_synthesis__statistics_statistics($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$_searchKey_=(($_nx_rec65=${$searchDataVar})!=null?$_nx_rec65->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					$records=
						$this->getRecords('','page.records','handler',false,$searchData,null);
					(($__o=$this->getHandler('','')) ? $__o->load_stats($records) : null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/dashboards/integrations_synthesis__stat',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/dashboards/integrations_synthesis__stat_ajax',array(
								'fmethod' => 'toHTML',
