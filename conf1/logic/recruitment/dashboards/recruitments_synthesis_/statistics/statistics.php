...
class recruitment_dashboards_recruitments_synthesis__statistics_statistics extends NxPage
{
	function recruitment_dashboards_recruitments_synthesis__statistics_statistics($props,&$parentDesc,$aspect)
	{
	...
	{
					$searchData=
						$this->getRecord('rct_vacancy.dashboards.recruitments_synthesis.search','page.search','request',false,null,null);
					$searchDataVar='searchData';
					$_searchKey_=(($_nx_rec69=${$searchDataVar})!=null?$_nx_rec69->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					$records=
						$this->getRecords('rct_vacancy.dashboards.recruitments_synthesis.list','page.records','handler',false,$searchData,null);
					(($__o=$this->getHandler('lib.recruitment.dashboards.services.dashboard','')) ? $__o->load_stats($records) : null);
					$this->setProperty('search.url',$_SERVER["PHP_SELF"].'?nxo=statistics&amp;nxs=&amp;filterKey='.urlencode($this->getProperty("filterKey","")).'&amp;mode=ajax');
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/dashboards/recruitments_synthesis__stat',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/dashboards/recruitments_synthesis__stat_ajax',array(
								'fmethod' => 'toHTML',
