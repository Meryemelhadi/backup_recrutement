...
class recruitment_dashboards_recruitments_synthesis__list_display extends NxPage
{
	function recruitment_dashboards_recruitments_synthesis__list_display($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/dashboards/recruitments_synthesis__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/dashboards/recruitments_synthesis__list_ajax',array(
								'fmethod' => 'toHTML',
