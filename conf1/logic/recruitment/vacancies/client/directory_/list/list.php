...
class recruitment_vacancies_client_directory__list_list extends NxPage
{
	function recruitment_vacancies_client_directory__list_list($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$_searchKey_=(($_nx_rec4=${$searchDataVar})!=null?$_nx_rec4->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					$this->getRecords('','tri.records','db',false,null,null);
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/vacancies/client/directory__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/vacancies/client/directory__list_ajax',array(
								'fmethod' => 'toHTML',
