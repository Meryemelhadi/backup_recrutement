...
class recruitment_vacancies_admin_directory__list_list extends NxPage
{
	function recruitment_vacancies_admin_directory__list_list($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$_searchKey_=(($_nx_rec46=${$searchDataVar})!=null?$_nx_rec46->getUrlKey(true):'');
			...
			$GLOBALS["NX_CURRENT_URL"] ='?'.$_searchKey_;
					...
					if($_nx_records51=$vacanciesList) {
					for (	$_nx_iterRec50 = $vacanciesList->getRecordIterator(); 
						$_nx_iterRec50->isValid(); 
						$_nx_iterRec50->next())
					{
					$vacancy=&$_nx_iterRec50->getCurrent();
						...
						$candidatescount=
							(($_nx_records47=$candidatesList)!=null?$_nx_records47->count():0);
						if(($_nx_rec48=&$vacancy)!=null) $_nx_rec48->setField('candidates_count',$candidatescount);
					}}
					...
					if(in_array((isset($_GET["mode"])?$_GET["mode"]:""),array('page','ajax')))
					{
						...
						if($viewMode == 'page')
						{
							$this->runView('/recruitment/vacancies/admin/directory__list',array(
								'fmethod' => 'toHTML',

								),null);
						}
						else {
							$this->runView('/recruitment/vacancies/admin/directory__list_ajax',array(
								'fmethod' => 'toHTML',
