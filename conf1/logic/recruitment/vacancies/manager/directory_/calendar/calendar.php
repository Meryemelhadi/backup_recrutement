...
class recruitment_vacancies_manager_directory__calendar_calendar extends NxPage
{
	function recruitment_vacancies_manager_directory__calendar_calendar($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$this->getRecords('rct_interview.validation_list','page.records','db',false,null,null);
					$this->setProperty('interviews_list.url',$_SERVER["PHP_SELF"].'?nxo=interviews_list&amp;nxs=&amp;oid='.(isset($_REQUEST["oid"])?$_REQUEST["oid"]:""));
					$this->setProperty('add_interview.url',$_SERVER["PHP_SELF"].'?nxo=add_interview&amp;nxs=&amp;oid='.(isset($_REQUEST["oid"])?$_REQUEST["oid"]:""));
					$this->setProperty('validate_interview.url',$_SERVER["PHP_SELF"].'?nxo=validate_interview&amp;nxs=&amp;oid='.(isset($_REQUEST["oid"])?$_REQUEST["oid"]:""));
					$this->runView('/recruitment/vacancies/manager/directory__calendar',array(
						'fmethod' => 'toHTML',
