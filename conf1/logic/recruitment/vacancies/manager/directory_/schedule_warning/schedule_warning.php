...
class recruitment_vacancies_manager_directory__schedule_warning_schedule_warning extends NxPage
{
	function recruitment_vacancies_manager_directory__schedule_warning_schedule_warning($props,&$parentDesc,$aspect)
	{
	...
	function run()
	{
					...
					$this->setProperty("vacancy_oid",(isset($_REQUEST["vacancy_oid"])?$_REQUEST["vacancy_oid"]:""));
					$this->getRecord('rct_warning.schedule@recruitment','page.record','empty',false,null,null);
					$this->setProperty('previous_operation.url',$_SERVER["PHP_SELF"].'?nxo=warning&amp;nxs=&amp;oid='.$this->getProperty("vacancy_oid",""));
					$this->setProperty('submit.url',$_SERVER["PHP_SELF"].'?nxo=schedule_warning_process&amp;nxs=&amp;vacancy_oid='.$this->getProperty("vacancy_oid",""));
					$this->runView('/recruitment/vacancies/manager/directory__warning_schedule',array(
						'fmethod' => 'toHTML',
